...
class LatentDiffusion_audio(DDPM):
    ...
    # def get_first_stage_encoding_vocal(self, encoder_posterior):
    #     if isinstance(encoder_posterior, DiagonalGaussianDistribution):
    #         z = encoder_posterior.sample()
    #     elif isinstance(encoder_posterior, torch.Tensor):
    #         z = encoder_posterior
    ...
    # @torch.no_grad()
    # def encode_first_stage_vocal(self, x):
    #     return self.first_stage_model.encode_vocal(x)
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        x_recon,lb_loss = self.model(x_noisy, t, **cond)

        if isinstance(x_recon, tuple) and not return_ids:
            return x_recon[0],lb_loss
        else:
            return x_recon,lb_loss
    ...
    def p_mean_variance(self, x, c, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
        ...
        t_in = t
        model_out,loss = self.apply_model(x, t_in, c, return_ids=return_codebook_ids)
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if plot_progressive_rows:
            with self.ema_scope("Plotting Progressives"):
                ...
                mel_length = math.ceil(c['acoustic']['acoustic'].shape[2] * 1 / 2)
                shape = (self.channels, self.mel_dim, mel_length) if self.channels > 0 else (self.mel_dim, mel_length)
