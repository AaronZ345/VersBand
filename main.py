...
import ldm.data
import ldm.data.joinaudiodataset_anylen
...
class DataModuleFromConfig(pl.LightningDataModule):# batchloader outputshape should be (b,h,w,c) and it will be permuted to (b,c,h,w) in autoencoder.get_input()
    ...
    def _train_dataloader(self):
        ...
        if isinstance(self.datasets["train"],ldm.data.joinaudiodataset_anylen.JoinManifestSpecs):
            ...
            return loader
        elif isinstance(self.datasets["train"], ldm.data.vocal2accomp_dataset.JoinManifestSpecs):
            from ldm.data.vocal2accomp_dataset import DDPIndexBatchSampler
            dataset = self.datasets["train"]
            ...
            print("train_loader_length", len(loader))
            return loader
        elif isinstance(self.datasets["train"], ldm.data.vocal2accomp_musical_dataset.JoinManifestSpecs):
    ...
    def _val_dataloader(self, shuffle=False):
        ...
        if isinstance(self.datasets["validation"],ldm.data.joinaudiodataset_anylen.JoinManifestSpecs):
            ...
            return DataLoader(dataset, batch_sampler=batch_sampler,sampler=None,
                          num_workers=self.num_workers, collate_fn=dataset.collater,
                          worker_init_fn=init_fn)
        elif isinstance(self.datasets["validation"],ldm.data.vocal2accomp_dataset.JoinManifestSpecs):
            from ldm.data.vocal2accomp_dataset import DDPIndexBatchSampler
            dataset = self.datasets["validation"]
            main_indices = dataset.ordered_indices()
