...
def parse_args():
    ...
    parser.add_argument(
        "--other_condition",    
        type=str,
        default='/root/autodl-tmp/data/manifests/vocal_to_accomp/train/v2c_0905/f0.npy'
    )
    parser.add_argument(
        "--ddim_steps",
        type=int,
...
class InferDataset(Dataset):
    def __init__(self, manifest_path,other_condition):
        ...
        for item_name in self.pred_list:
            ...
            if float(item['duration']) > 20:
                continue
            item['f0'] = self.f0_dict[item_name]
    ...
    def __getitem__(self, index):
        ...
        org_spec_len = spec_len = spec.shape[1]
        f0 = np.expand_dims(data['f0'], axis=0) # [1, T]
        ...
        f0 = pad_or_cut_xd(torch.FloatTensor(f0), acoustic_len, dim=1, pad_value=0)
    ...
    for batch in loader:
        ...
        for scale in scales:
            ...
            condition = {
                ...
                'acoustic': {'acoustic':torch.stack([acoustic] * args.n_samples),'f0':torch.stack([f0] * args.n_samples),'midi':torch.stack([midi] * args.n_samples).long(),'beats':torch.stack([beats] * args.n_samples).long(),
                             },
