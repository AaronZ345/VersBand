    ...
    for ds_name in data_paths:
        ...
        for item in tqdm(manifest_items, ncols=80, desc=ds_name, total=len(manifest_items)):
            ...
            if abs(midi_time- float(item['wav_len']))>1:
                print(f'| Skip {item_name}: midi for {midi_time},{item["wav_len"]}')
                assert False
            ...
            assert len(beat_frames) == vocal_mel.shape[1] == len(frame_pitches) ==accomp_mel.shape[1],f'{len(beat_frames)},{vocal_mel.shape[1]},{len(frame_pitches)},{accomp_mel.shape[1]}'
            wav_path = new_item['audio_path']
